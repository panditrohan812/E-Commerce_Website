/* =========================================================
   SHOPINGO - ALL PRODUCTS PAGE
   Fetches full catalogue, filters/sorts/paginates client-side.
   ========================================================= */
...
function bindControls() {
  ...
  if (priceRange) {
    priceRange.addEventListener("input", (e) => {
      ...
      document.getElementById("priceRangeValue").textContent = `$${state.maxPrice}`;
      state.page = 1;
      applyAndRender();
    });
  }
  ...
  if (clearBtn) {
    clearBtn.addEventListener("click", () => {
      ...
      state.sort = "default";
      state.page = 1;
      if (priceRange) { priceRange.value = 1000; document.getElementById("priceRangeValue").textContent = "$1000"; }
...
function updatePageHeader() {
  const title = state.activeCategories.size === 1
...
function applyAndRender() {
  ...
  items = items.filter((p) => (p.rating ? p.rating.rate : 0) >= state.minRating);

  switch (state.sort) {
    case "price-asc": items.sort((a, b) => a.price - b.price); break;
    case "price-desc": items.sort((a, b) => b.price - a.price); break;
    case "rating-desc": items.sort((a, b) => (b.rating?.rate || 0) - (a.rating?.rate || 0)); break;
  ...
  const grid = document.getElementById("productsGrid");
  grid.className = state.view === "grid"
